...
class PypiPackageRef(HashableStruct, frozen=True, tag=True):
    "A reference to Python package assumed to be published in PyPI."
...
class NixFlake(msgspec.Struct, frozen=True, tag=True):
    ...
    def __post_init__(self):
        for path in self.files:
            ...
            if ".." in p.parts:
                raise ValueError(
                    f"Path '{path}' contains '..': security risk (path traversal)"
                )
